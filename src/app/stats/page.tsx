          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {topTags.map(([tag, n]) => (
              <Link
                key={tag}
                href={`/tags/${tag}`}
                className="rounded-full border border-rule bg-surface px-3 py-1.5 text-xs uppercase tracking-widest transition-colors hover:border-accent hover:text-accent"
              >
                #{tag}{' '}
