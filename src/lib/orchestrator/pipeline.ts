 ...
 * trending sources. Skips stages 1–2 (gather/score), synthesizes a "winner"
 * from the topic string, and runs the same research → generate → image →
 * serialize path as `runPipeline`, so the output is identical in shape. Needs
 * the LLM key and `BRAVE_API_KEY` (the topic has no source URL of its own, so
 * research relies on web search).
 */
